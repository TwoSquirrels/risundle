...
keep (設定ファイル・CLI とも) に登録済みライブラリと一致しない ID が含まれる場合、および `--no-keep` の ID が (設定の keep ∪ `--keep`) のどれにも一致しなかった場合も、効果を持たない指定 (no-op) として警告を出す。`std` は前段の警告の管轄のため対象外。エラーにはしない: 設定ファイルはリポジトリにコミットされる一方でライブラリ登録はマシンローカルな状態であり、clone 直後の未登録 ID は構造的に生じるため。警告は複数 ID を 1 行に集約し、出力の生成を妨げず、終了コードも変えない。
