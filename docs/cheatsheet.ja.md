...
これで直る場合は tree-shaking が必要な定義を削っています。既知の取りこぼしは、実装先を静的に特定できない実装ファイル (自由関数の演算子だけを定義するファイルなど) です。それ以外のケースは [Issue](https://github.com/TwoSquirrels/risundle/issues) で報告してもらえると助かります。この切り替えを自動化したい場合は「[コンパイルが通ることを確認してから提出したい](#コンパイルが通ることを確認してから提出したい-自動フォールバック)」を参照してください。これでも直らない場合は tree-shaking 以外の原因なので、コンパイラのエラーメッセージから直接原因を辿ってください。
